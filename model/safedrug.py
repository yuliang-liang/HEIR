...
# 获得句向量
def get_icd_description_embedding_dict(tokens,vocab="ICD9CM"):
    #  tokens is conditions/procedure is the list of icd code 
    ...
    for code in tokens:
        dsc = get_icd_description(code,vocab)
        desc_list.append(dsc)
    
    inputs = tokenizer(desc_list, return_tensors='pt', max_length=512, truncation=True, padding=True)['input_ids']
    # [n, squence, hidden_dim]
    last_hidden_state = model(inputs).last_hidden_state
    # cls embedding
    cls = last_hidden_state[:, 0, :]
    desc_emb_dict = {}
    for i, code in enumerate(tokens):
        desc_emb_dict[code] = cls[i]
    return desc_emb_dict




class SafeDrug_ICD(BaseModel):
    """SafeDrug model.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SafeDrug_ICD, self).__init__(
            dataset=dataset,
        ...
        #discrete icd encoder
        tokens = list(self.feat_tokenizers['conditions'].vocabulary.idx2token.values())[2:]
        self.cond_icd_dict = discretize_icd_code_dictionary(tokens)
        tokens  = list(self.feat_tokenizers['procedures'].vocabulary.idx2token.values())[2:]
        self.proc_icd_dict = discretize_icd_code_dictionary(tokens)

        # discrete icd embedding
        cond_encoding_length = len(list(self.cond_icd_dict.values())[0])
        self.cond_dense = nn.Linear(cond_encoding_length, embedding_dim)
        proc_encoding_length = len(list(self.proc_icd_dict.values())[0])
        self.proc_dense = nn.Linear(proc_encoding_length, embedding_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # ------------------discretize icd code----------------------
        # (patient, visit, code, discrete_icd_feature)
        conditions_icd = torch.zeros(conditions.shape[0], conditions.shape[1], conditions.shape[2],
                                     len(list(self.cond_icd_dict.values())[0]),
                                     dtype=torch.float, 
                                     device=self.device
                                     )
        # 获取非padding token的索引
        non_padding_mask = conditions != self.feat_tokenizers["conditions"].vocabulary.token2idx['<pad>']
        non_padding_indices = non_padding_mask.nonzero(as_tuple=True)
        # 获取非padding token在visit中的编码
        non_padding_tokens = conditions[non_padding_mask]
        non_padding_icd = torch.tensor(
            [self.cond_icd_dict[self.feat_tokenizers["conditions"].vocabulary.idx2token[idx.item()]] 
            for idx in non_padding_tokens], dtype=torch.float, device=self.device
            )
        # 将非padding token的编码放置回conditions_icd中
        conditions_icd[non_padding_indices[0], non_padding_indices[1], non_padding_indices[2], :] = non_padding_icd
        
        # for each token in the visit, get the encoding
        # for i, patient in enumerate(conditions):
        #     for j, visit in enumerate(patient):
        #         for k, id in enumerate(visit):
        #             token = self.feat_tokenizers["conditions"].vocabulary.idx2token[id.item()]
        #             if token != '<pad>':
        #                 conditions_icd[i,j,k,:] = torch.tensor(self.cond_icd_dict[token])


        
        # (patient, visit, code, embedding_dim)
        conditions = self.cond_dense(conditions_icd)
        # (patient, visit, embedding_dim)
        conditions = torch.sum(conditions, dim=2)
        ...
        # ------------------discretize icd code----------------------
        # (patient, visit, code, discrete_icd_feature)
        procedures_icd = torch.zeros(procedures.shape[0], procedures.shape[1], procedures.shape[2],
                                     len(list(self.proc_icd_dict.values())[0]),
                                     dtype=torch.float, 
                                     device=self.device
                                     )
        # 获取非padding token的索引
        non_padding_mask = procedures != self.feat_tokenizers["procedures"].vocabulary.token2idx['<pad>']
        non_padding_indices = non_padding_mask.nonzero(as_tuple=True)
        # 获取非padding token在visit中的编码
        non_padding_tokens = procedures[non_padding_mask]
        non_padding_icd = torch.tensor(
            [self.proc_icd_dict[self.feat_tokenizers["procedures"].vocabulary.idx2token[idx.item()]] 
            for idx in non_padding_tokens], dtype=torch.float, device=self.device
            )
        # 将非padding token的编码放置回procedures_icd中
        procedures_icd[non_padding_indices[0], non_padding_indices[1], non_padding_indices[2], :] = non_padding_icd


        # (patient, visit, code, embedding_dim)
        #procedures = self.embeddings["procedures"](procedures)
        procedures = self.proc_dense(procedures_icd)
        # (patient, visit, embedding_dim)

...
class RETAIN_ICD(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(RETAIN_ICD, self).__init__(
            dataset=dataset,
        ...
        # --------------------------------------ours--------------------------------------
        from model.safedrug_icd_unsup_pos import PositionEmbedding, GraphEmbedding
        ...
        # graph embedding
        self.cond_graph_embedding = GraphEmbedding("ICD10CM", 
                                                   embedding_dim, 
                                                   self.feat_tokenizers['conditions'].vocabulary.idx2token, 
                                                   semantic=False,
                                                   pos_embedding=self.cond_pos_embedding)
        self.proc_graph_embedding = GraphEmbedding("ICD10PROC",
                                                    embedding_dim,
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        for feature_key in self.feature_keys:
            ...
            # for case 2: [[code1, code2], [code3, ...], ...]
                ...
                old_x = self.embeddings[feature_key](x) 
                if feature_key == "conditions":
                    #x = self.cond_graph_embedding(x) + self.cond_pos_embedding(x)
                    x =  old_x +self.cond_graph_embedding(x) + self.cond_pos_embedding(x)
                if feature_key == "procedures":
                    #x = self.proc_graph_embedding(x) + self.proc_pos_embedding(x)
                    x =  old_x +self.proc_graph_embedding(x)+ self.proc_pos_embedding(x)

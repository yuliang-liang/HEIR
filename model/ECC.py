...
class ECC:
    ...
    def __init__(
        ...
        tables: List[str],
        target: str,
        #classifier: Optional[ClassifierMixin],
        mode: str,
        **kwargs
    ):
        ...
        self.chains = [
            ClassifierChain(self.classifier, order="random", random_state=i) for i in range(10)
        ]
    ...
    def fit(
        ...
        if self.mode == "multilabel":
            ...
            for i, chain in enumerate(self.chains):
                chain.fit(X, y[:, self.valid_label])
    ...
    def __call__(self, **kwargs):
        ...
        y_prob_chains = np.array(
            [chain.predict_proba(X) for chain in self.chains]
            )
        ...
        if self.mode == "multilabel":
            ...
            cur_prob = np.mean(y_prob_chains, axis=0)

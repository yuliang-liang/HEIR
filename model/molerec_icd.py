...
class MoleRecLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        GNN_layers: int = 4,
        dropout: float = 0.5,
        multiloss_weight: float = 0.0, #float = 0.05
        **kwargs,
    ):
...
class MoleRec_ICD(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(MoleRec_ICD, self).__init__(
            dataset=dataset,
    ...
    def encode_patient(
        ...
        if feature_key == "conditions":
            embeddings = self.cond_graph_embedding(codes) + self.cond_pos_embedding(codes)
        if feature_key == "procedures":
            embeddings = self.proc_graph_embedding(codes) + self.proc_pos_embedding(codes)
        #embeddings = self.embeddings[feature_key](codes)
        embeddings = torch.sum(self.dropout_fn(embeddings), dim=2)
        #embeddings = torch.sum(embeddings, dim=2)
